...
Nothing has shipped to external users. **Do not treat backward compatibility as a constraint.** If a public API needs to change to be correct or simpler, change it and update all call sites. Deprecation shims, re-exports for removed names, and compatibility hacks are waste — leave them out.
