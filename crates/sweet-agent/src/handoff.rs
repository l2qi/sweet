...
/// Result of an *interruptible* turn ([`crate::Agent::step_stream_interruptible`]
/// / [`crate::Agent::resume_with_approvals`]): either the turn finished (a
/// [`TurnResult`]) or it paused awaiting approval of one or more tool calls.
///
/// Kept separate from [`TurnResult`] so the plain [`crate::Agent::step`] API and
/// its many consumers stay unchanged; only callers that opt into the
/// interruptible API see `Paused`.
#[derive(Debug, Clone)]
