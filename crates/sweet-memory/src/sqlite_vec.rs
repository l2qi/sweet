...
use sweet_core::{
    unix_now, Embedder, Memory, MemoryError, MemoryHit, MemoryId, MemoryQuery, MemoryRecord,
    MemoryScope,
};
...
use crate::sqlite_shared::{
    embed_query, filter_record, fts_candidates, fuse_hits, list_newest, open_conn, row_to_record,
...
impl SqliteVecMemory {
    ...
    fn vector_candidates(
        ...
        for row in rows {
            ...
            // Filter by embedder model, scope, and tags in Rust.
...
impl Memory for SqliteVecMemory {
    ...
    async fn search(&self, query: &MemoryQuery) -> Result<Vec<MemoryHit>, MemoryError> {
        ...
        let query_embedding = embed_query(self.embedder.as_deref(), text)
            .await?
            .and_then(|(v, id)| self.check_dims(Some(v)).map(|v| (v, id)));
