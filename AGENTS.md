...
- An OpenAI-wire-compatible endpoint (OpenRouter, a local llama server, etc.) with no reasoning quirks is `OpenAIProvider` preconfigured with a base URL — not a new provider; such newtypes belong next to their consumer, not in `sweet-llm`. The exception is when the endpoint diverges on *reasoning* semantics beyond a base URL: `CerebrasProvider` earns its own dedicated provider because it rejects the `thinking` object, replays no reasoning history (Cerebras accepts neither `reasoning_content` nor `reasoning` on a request and 400s `wrong_api_format` if either is sent), and is effort-only — divergences that cannot be expressed by composition alone. A provider whose only difference is the base URL never earns a dedicated provider.
...
- `Message.thinking_content: Vec<ThinkingContent>` carries chain-of-thought blocks. `ThinkingContent` is `text` plus optional `signature`/`redacted_data` (Anthropic) and `raw` (a verbatim provider-native block, e.g. one OpenRouter `reasoning_details` entry, for exact round-trip). It is `PartialEq` but not `Eq` (so neither is `Message`).
...
Public surface: `OpenAIProvider`, `GeminiProvider`, `AnthropicProvider`, `CerebrasProvider`, `OpenAIEmbedder`, `GeminiEmbedder`, `ReasoningConfig`, `ReasoningHistoryKey`, `SamplingConfig`, `ToolChoice`, `StructuredOutput`, `ProviderError`. Reasoning is configured via the cross-provider `ReasoningConfig` (Toggle/Effort/Budget); each provider's `with_reasoning` maps each variant to a single wire shape. **The provider never sniffs the model name** - where a model family needs a different wire encoding, the caller (which owns the catalog) expresses it by choosing the variant: Anthropic `Toggle(true)` -> `{type: adaptive}` (Claude 4.6+) while older models take an explicit `Budget`; Gemini `Effort` -> `thinkingLevel` (Gemini 3+) while 2.5 takes a `Budget`.

- `OpenAIProvider` exposes reasoning via `reasoning_effort` and a `thinking` object (`with_reasoning`); `openai::ReasoningContent` is the single-block reasoning view over `thinking_content`. Replaying a prior turn's reasoning on a request is **opt-in and per-model**: `with_reasoning_history_key(ReasoningHistoryKey)` selects the wire field — `ReasoningContent` / `Reasoning` / `ReasoningDetails` (OpenRouter's structured array, round-tripped verbatim via `ThinkingContent::raw`) — and defaults to `Omit` (replay nothing). Catalog-driven callers set it from models.dev `interleaved`.
